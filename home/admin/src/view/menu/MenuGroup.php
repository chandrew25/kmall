...
class MenuGroup extends Viewable
{
     ...
     * 菜单配置文件
     */
    const CONFIG_ROLE_MENU_FILE="menu_role_play.xml";
	/**
	 * 编号
	 * @var string
	 */
	...
	*/
    ...
    {
        $uri=dirname(__FILE__).DIRECTORY_SEPARATOR.self::CONFIG_ROLE_MENU_FILE;
	...
	public static function viewForExtJs()
	{
		...
		if ($menuConfigs!=null)
		{
			foreach ($menuConfigs as $menuGroup)
			{
                ...
                {
                    //XMl数组对象转换为数组
                    $menu = UtilXmlSimple::objectsIntoArray($menu);
                    //for权限
                    $for = $menu['@attributes']['for'];
                    $forlist = explode(",",$for);
                    if ($admin->roletype!="0"){
                        if (in_array($menu['@attributes']['id'],$authority)||in_array($admin->roletype,$forlist)){
                           $isCanIn=true;
                           break;
                        }
                    }else{
