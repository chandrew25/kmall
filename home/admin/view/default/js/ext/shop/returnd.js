    ...
    Grid:Ext.extend(Ext.grid.GridPanel, {
                        ...
                        new Ext.Toolbar({
                            ...
                            items : [
                                ...
                                {ref: '../ostatus',xtype : 'combo',mode : 'local',width : 80,
                                    ...
                                    store : new Ext.data.SimpleStore({
                                        fields : ['value', 'text'],
                                        data : [['active', '有效'],['finish', '完成'],['dead', '无效']]
                                      }),
                                    valueField : 'value',// 值
                                    displayField : 'text'// 显示文本
                                },
                                ...
                                {ref: '../opay_status',xtype : 'combo',mode : 'local',width : 100,
                                    ...
                                    store : new Ext.data.SimpleStore({
                                        fields : ['value', 'text'],
                                        data : [['5','全额退款'],['9', '部分退款'],['0', '准备支付'],['1', '支付成功'],['10', '已收款']]
                                      }),
                                    valueField : 'value',// 值
                                    displayField : 'text'// 显示文本
                                },
                            ...
                            items : [
                                {
                                    ...
                                    text : '确认部分退货',ref: '../../btnPartReturn',iconCls : 'icon-edit',disabled : true,
                                    handler : function() {
                                        this.returnPartGood();
                                    }
                                },{
        ...
        sm : new Ext.grid.CheckboxSelectionModel({
            ...
            listeners : {
                selectionchange:function(sm) {
                    var data;
                    if (sm.getSelected()) data = sm.getSelected().data;
                    // 判断更新按钮是否可以激活
                    this.grid.btnReturn.setDisabled(sm.getCount() != 1 || (data && data.ship_status!=7));
                    this.grid.btnPartReturn.setDisabled(sm.getCount() != 1 || (data && data.ship_status!=7));
                    this.grid.btnRefund.setDisabled(sm.getCount() != 1 || (data && data.pay_status==5));
        ...
        doSelectOrder : function() {
            ...
            Ext.apply(condition,this.filter);
            ExtServiceOrder.queryPageReturnd(condition,function(provider, response) {
...
/*                        for ( var flag = 0; flag < selectedRows.length; flag++) {
         ...
         * 提示:退货
         */
        returnPartGood:function(){
            Ext.Msg.confirm('提示', '是否确认选择的订单已退货?', this.returnPartGoodConfirm, this);
        },
        /**
         * 退货
         */
        returnPartGoodConfirm:function(btn){
            if (btn == 'yes') {
                var check_ids ="";
                var selectedRows    = this.getSelectionModel().getSelections();
                for ( var flag = 0; flag < selectedRows.length; flag++) {
                ...
                ExtServiceOrder.returnPartGood(check_ids);
                Km.Order.View.Running.orderGrid.doSelectOrder();
                Ext.Msg.alert("提示", "操作成功,选择的订单已确认退货！");
            }
        },
...
Ext.onReady(function(){
    ...
    Km.Order.Store.orderStore.proxy=new Ext.data.DirectProxy({
        api: {read:ExtServiceOrder.queryPageReturnd}
    });
    /**
     * 订单页面布局
     */
