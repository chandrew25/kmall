...
{        
	...
	public function index()
	{
		...
		$ptype_goods = Ptype::get("isShow=1 and level=1 ","sort_order desc","1,2");
        if (!empty($ptype_goods)){
            foreach ($ptype_goods as $key=>$value){
                $ptype_goods[$key]['goods']=Product::get("isShow=1 and isUp=1 and ptype_key like '%-".$value->ptype_id."-%'",'sort_order desc','1,12');
            }
        }
