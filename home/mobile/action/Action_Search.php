...
class Action_Search extends ActionMobile
{
    ...
    public function lists()
    {
        ...
        if ( $websafe ) {
            ...
            $isMultitable = true;
            if ( $count <= 0 ) {
                $count = sqlExecute("select count(a.product_id) from km_product a where a.product_name like '%".$keyword."%' and price_tag is not null and a.isUp=1 and a.isShow=1");
                $isMultitable = false;
            }
            ...
            if ( $isMultitable ) {
                ...
                $where_clause = " product_name like '%" . $keyword . "%' and price_tag is not null and isUp=1 and isShow=1";
