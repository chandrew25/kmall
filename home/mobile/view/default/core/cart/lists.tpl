	...
	<div id="shopp_cart">
			...
			<dl class="supp_main clearfix">
				...
				<dd class="supp_con">
					...
					<div class="supp_c_price clearfix">
						<span class="supp_box">￥{$cart->sales_price*$cart->num|string_format:'%d'}+{$cart->jifen*$cart->num|default:'0'}券</span>
						<p class="supp_c_inpt">
		...
		<div class="con_sub cart_che clearfix">
			<span class="con_color cart_main fl">
				...
				{if $foo}
				<font class="cart_money">(含综合服务费:￥{$foo})</font>
				{/if}
			</span>
			<a href="{$url_base}index.php?go=mobile.checkout.view" class="con_ti fr">确认提交</a>
		</div>
	</div>
{/block}
