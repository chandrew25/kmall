...
{block name=body}
	<div id="wrapper" class="m_pwd">
		...
		<div class="int_title">
			<span class="int_pic">
				<a href="{$url_base}index.php?go=mobile.member.view">
		...
		<div class="order">
			...
			<dl class="order_box topline clearfix">
				<dt class="order_pic fl">
					<img src="{$upload_url}images/{$goods->image}" />
				</dt>
			...
			<div class="order_btn topline clearfix">
				<p class="order_t_box clearfix fr">
					<a href="{$url_base}index.php?go=mobile.order.info&id={$order->order_id}" class="p_money order_border order_style fl">订单详情</a>
					{if $order->pay_status==0}
					<a href="{$url_base}index.php?go=mobile.checkout.show&order_id={$order->order_id}" class="p_money order_border  fr">付款</a>
					{else}
