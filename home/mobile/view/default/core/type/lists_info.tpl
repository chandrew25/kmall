...
{*<div id="wrapper" class="inner_style">*}
    <div class="int_title">
        <span class="int_pic">
            {* <a href="{$url_base}index.php?go=mobile.type.lists"> *}
    ...
    {*</div>*}
    <div class="type_list">
        <div class="type_list_title">三级分类</div>
        <ul>
            {foreach from=$ptype item=top_ptype key=index}
            <li class="{if $ptypeid==$top_ptype.ptype_id}inner_con_style{/if}">
                <a class="tl_title" href="{$url_base}index.php?go=mobile.type.lists_info&ptype_id={$ptype_id}&ptypeid={$top_ptype.ptype_id}">
                {$top_ptype.name}
                </a>
            </li>
            {/foreach}
            ...
            <ul style="display: block;" class="clearfix best_content">
                ...
                <a href="{$url_base}index.php?go=mobile.goods.info&id={$p->product_id}">
                    <li class="border_right fl">
                        ...
                        {else}
                        <div style='height:180px;width:180px;line-height:180px;text-align:center;'>暂无图片</div>
                        {/if}
                                ...
                                if (countAll > currentCount){
                                    for (var i = 0; i < data.length; i++) {
                                        ...
                                        result += "    <a href='" + url_base + "index.php?go=mobile.goods.info&id=" + product.product_id + "'>" +
                                                  "       <li class='border_right fl'> " +
                                                  "          <img src='" + product.image + "' style='min-width:180px; min-height:180px;' alt='暂无图片' /> " +
