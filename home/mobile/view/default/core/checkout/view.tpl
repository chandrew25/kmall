        ...
        <div class="fill_main">
            ...
            <dl class="fill_dl clearfix">
                ...
                <dt class="fill_p_pic fl">
                    <img src="{$upload_url}images/{$cart->ico}" />
                </dt>
                <dd class="fill_p_text fr">
                    <span class="fill_p_con">{$cart->goods_name}</span>
                    <p class="fill_p"><span class="f_right fr">x{$cart->num}</span></p>
                    <span class="fill_int">￥{$cart->sales_price|string_format:'%d'}+{$cart->jifen}券</span>
                </dd>
            </dl>
            {/foreach}
        </div>
        <div class="con_sub clearfix" style="margin-bottom: 10px;">
            ...
            <span class="con_color fill_style fl all_style">
                    合计:<font>￥{$statisticAll.realFee}{if $statisticAll.totalJifen}+{$statisticAll.totalJifen}券{/if}</font>
                    {if $statisticAll.deliveryFee}
                    <font class="cart_money">(含综合服务费:￥{$statisticAll.deliveryFee})</font>
                    {/if}
            </span>
