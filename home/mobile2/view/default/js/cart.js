function cartNum(cartId,num,t){
    ...
    $.ajax({
        url: "index.php?go=mobile2.cart.addCart",
        type: 'post',
        dataType: 'json',
        data:data,
        success: function(response) {
            if (response.success=="success") {
                window.location.href="index.php?go=mobile2.cart.lists";
            }else{
                alert(response.data);
                return false;
            }
...
$(document).ready(function(){
    ...
    $(".del_goods").bind("click",function(){
        ...
        if (isConfim) {
            $.ajax({
                url: "index.php?go=mobile2.cart.delCart",
                type: 'post',
                dataType: 'json',
                data:data,
                success: function(response) {
                    if (response.success=="success") {
                        location.href = "index.php?go=mobile2.cart.lists";
                    }else{
    ...
    $(".checkout").bind("click",function(){
        ...
        $.ajax({
            url: "index.php?go=mobile2.member.checkOrder",
            type: 'post',
            dataType: 'json',
            data:data,
            success: function(response) {
