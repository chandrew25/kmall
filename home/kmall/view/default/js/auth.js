...
$(function(){
  $("#btnSmsSend").click(function(){
      ...
      // var smsSendUrl="home/kmall/src/httpdata/smsSend.php?mobile="+mobile;
      // $.get(smsSendUrl);
  ...
  $("#smsPwd").keyup(function(){
	...
	$("#agree").click(function(){
		...
		if(checked){
			$("#btn_register").removeClass("b");
		}else{
			$("#btn_register").addClass("b");
		}
	});
	/*************注册按钮*************/
	...
	$("#username").blur(function(){
		if(checkUsername()){
			...
			$.post(root+"index.php?go=kmall.ajax.checkUsername",data,function(result){
				if(result=="1"){
					$("#message").html("该用户名已有用户注册");
				}else{
