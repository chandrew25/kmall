 ...
 */
class Action_Checkout extends Action
{
    /**
     * 结算页面
     */
    ...
    public function viewOff()
    {
        ...
        foreach ($sels as $cart_id=>$checked) {
            if($checked=='on'){
                ...
                $cartData[$cart_id] = $cart;
            }
        }
        //显示购物车商品列表信息
        $this->view->set("carts",$cartData);
...
//        }
        ...
        $nav_info->level = 2;
        $nav_info->info = "订单";
        $nav_info->link = Gc::$url_base."index.php?go=kmall.checkout.view&location=cart";
    ...
    */
    public function overOff()
    {
        ...
        if ($pbankcode=="bank" || $pbankcode=="alipay" || $pbankcode=="wxpay"){
            ...
            $rankjifenlog->discribe="购买商品使用积分支付";
            $rankjifenlog->save();
    ...
    public function show()
    {
        ...
        if($payment_code=="alipay"||$payment_code=="reserve"){//支付宝
            ...
            include_once(Gc::$nav_root_path . "module/plugins/pay/wxpay/pagepay/pagepay.php");
            die();
            ...
            echo "支付出错了！";
            echo "<a href='".Gc::$url_base."index.php?go=kmall.member.view'>点击返回个人中心</a>";
        }
    }
