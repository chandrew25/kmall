 ...
 */
class Action_Display extends Action
{
	/**
	 * 商品类型列表页面
	 */
	...
	{   
        ...
        $websafearr = array();
        //判断变量类型并存入数组
        $websafearr[0] = is_numeric($attribute1_id)||empty($attribute1_id); 
        $websafearr[1] = is_numeric($brand_id)||empty($brand_id);
        $websafearr[2] = is_numeric($sorting)||empty($sorting);
        $websafearr[3] = is_numeric($ptype_id)||empty($ptype_id);
        $websafearr[4] = !contain($attr_key,"<script");
        $websafearr[5] = is_numeric($attribute_id)||empty($attribute_id);
        //判断是否存在false
        $websafe = in_array(false,$websafearr);
        if(!$websafe){
            ...
            //接受属性分类
            $attr_key = $this->data["attr_key"];
            $attr_type = sqlExecute("select distinct a.attribute1_id from ".Ptypeattr::tablename()." a where a.ptype_id=".$ptype_id." and a.attribute1_id order by ptypeattr_id asc");
            $attr_cut = array();
            $attr_selected = array();
            //获取该分类的属性分类
            for($i=0;$i<count($attr_type);$i++){
                $attr_cut[$i] = $attr_type[$i];
                $attr_selected[$attr_type[$i]] =0;
            }
            //属性分类字符串
            if(!$attr_key){
            ...
            //获取属性列表
            $attributes = sqlExecute("select distinct ".Attribute::tablename().".attribute_name ,".Attribute::tablename().".attribute_id from ".Attribute::tablename()." INNER JOIN ".Ptypeattr::tablename()." ON ".Ptypeattr::tablename().".attribute1_id = ".Attribute::tablename().".attribute_id where ".Ptypeattr::tablename().".ptype_id=".$ptype_id,Attribute::classname_static());
            ...
            $nav_info->level = 2;
            $nav_info->info = "产品一览";
            $nav_info->link = Gc::$url_base."index.php?go=kmall.display.lists";
