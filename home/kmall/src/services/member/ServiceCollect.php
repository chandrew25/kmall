 ...
 */
class ServiceCollect extends Service implements IServiceBasic
{
	/**
	 * 保存数据对象:会员收藏商品
	 * @param array|DataObject $collect
	 * @return int 保存对象记录的ID标识号
	 */
	public function save($collect)
	{
		if (is_array($collect)){
	 ...
	 */
	public function update($collect)
	{
		if (is_array($collect)){
			$collect=new Collect($collect);
		}
	...
	/**
	 * 由标识删除指定ID数据对象 :会员收藏商品
	 * @param int $id 数据对象:会员收藏商品标识
	 * @return boolen 是否删除成功；true为操作正常
	 */
	public function deleteByID($id)
	{
		return Collect::deleteByID($id);
	}

	/**
	 * 根据主键删除数据对象:会员收藏商品的多条数据记录
	...
	{
		return Collect::deleteByIds($ids);
	}
	...
	{
		return Collect::increment($filter,$property_name,$incre_value);
	}
	...
	{
		return Collect::decrement($filter,$property_name,$decre_value);
	}

	/**
	 * 查询数据对象:会员收藏商品需显示属性的列表
	...
	{
		return Collect::get($filter,$sort,$limit);
	}
	 ...
	 * 3.允许对象如new User(id="1",name="green");<br/>
	 * 默认:SQL Where条件子语句。如："(id=1 and name='sky') or (name like 'sky')"<br/>
	 * @return 数据对象:会员收藏商品总计数
	 */
	public function count($filter=null)
	{
		return Collect::count($filter);
	}
	...
	{
		return self::dao()->sqlExecute("select * from ".Collect::tablename(),Collect::classname_static());
	}
}
?>