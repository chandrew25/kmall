 ...
 */
class ServiceAddress extends Service implements IServiceBasic
{
	/**
	 * 保存数据对象:会员收货人地址信息
	...
	/**
	 * 由标识删除指定ID数据对象 :会员收货人地址信息
	 * @param int $id 数据对象:会员收货人地址信息标识
	...
	{
		return Address::deleteByIds($ids);
	}

	/**
	 * 对数据对象:会员收货人地址信息的属性进行递增
	...
	{
		return Address::increment($filter,$property_name,$incre_value);
	}

	/**
	 * 对数据对象:会员收货人地址信息的属性进行递减
	 * @param string $filter 查询条件，在where后的条件<br/>
	 * 示例如下：<br/>
	 * 0."id=1,name='sky'"<br/>
	 * 1.array("id=1","name='sky'")<br/>
	...
	{
		return Address::decrement($filter,$property_name,$decre_value);
	}

	/**
	 * 查询数据对象:会员收货人地址信息需显示属性的列表
	...
	{
		return Address::get($filter,$sort,$limit);
	}

	/**
	 * 查询得到单个数据对象:会员收货人地址信息实体
	 * @param string $filter 查询条件，在where后的条件<br/>
	 * 示例如下：<br/>
	 * 0."id=1,name='sky'"<br/>
	 * 1.array("id=1","name='sky'")<br/>
	 ...
	 * 3.允许对象如new User(id="1",name="green");<br/>
	 * 默认:SQL Where条件子语句。如："(id=1 and name='sky') or (name like 'sky')"<br/>
	 * @return 单个数据对象:会员收货人地址信息实体
	 */
	public function get_one($filter=null)
	{
		return Address::get_one($filter);
	}
	 ...
	 * 3.允许对象如new User(id="1",name="green");<br/>
	 * 默认:SQL Where条件子语句。如："(id=1 and name='sky') or (name like 'sky')"<br/>
	 * @return 数据对象:会员收货人地址信息总计数
	 */
	public function count($filter=null)
	{
		return Address::count($filter);
	}
	...
	{
		return Address::queryPage($startPoint,$endPoint,$filter,$sort);
	}
	/**
	 * 直接执行SQL语句
	 * @return array
	...
	{
		return self::dao()->sqlExecute("select * from ".Address::tablename(),Address::classname_static());
	}
}
?>