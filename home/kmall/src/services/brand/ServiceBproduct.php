 ...
 */
class ServiceBproduct extends Service implements IServiceBasic
{
    /**
     * 保存数据对象:品牌推荐商品
    ...
    {
        if (is_array($bproduct)){
            $bproduct=new Bproduct($bproduct);
        }
        if ($bproduct instanceof Bproduct){
            return $bproduct->save();
        }else{
            return false;
        }
    }
    ...
    /**
     * 由标识删除指定ID数据对象 :品牌推荐商品
     * @param int $id 数据对象:品牌推荐商品标识
     * @return boolen 是否删除成功；true为操作正常
     */
    public function deleteByID($id)
    {
        return Bproduct::deleteByID($id);
    }
    ...
    {
        return Bproduct::deleteByIds($ids);
    }
    ...
    {
        return Bproduct::increment($filter,$property_name,$incre_value);
    }
    ...
    {
        return Bproduct::decrement($filter,$property_name,$decre_value);
    }

    /**
     * 查询数据对象:品牌推荐商品需显示属性的列表
    ...
    {
        return Bproduct::get($filter,$sort,$limit);
    }
     ...
     * 3.允许对象如new User(id="1",name="green");<br/>
     * 默认:SQL Where条件子语句。如："(id=1 and name='sky') or (name like 'sky')"<br/>
     * @return 单个数据对象:品牌推荐商品实体
     */
    public function get_one($filter=null)
    {
        return Bproduct::get_one($filter);
    }
     ...
     * 3.允许对象如new User(id="1",name="green");<br/>
     * 默认:SQL Where条件子语句。如："(id=1 and name='sky') or (name like 'sky')"<br/>
     * @return 数据对象:品牌推荐商品总计数
     */
    public function count($filter=null)
    {
        return Bproduct::count($filter);
    }
    ...
    {
        return Bproduct::queryPage($startPoint,$endPoint,$filter,$sort);
    }
    /**
     * 直接执行SQL语句
     * @return array
    ...
    {
        return self::dao()->sqlExecute("select * from ".Bproduct::tablename(),Bproduct::classname_static());
    }
}
?>