 ...
 */
class ServiceBrandptype extends Service implements IServiceBasic
{
    /**
     * 保存数据对象:品牌商品分类关系
    ...
    public function save($brandptype)
    {
        ...
        if ($brandptype instanceof Brandptype){
            return $brandptype->save();
        }else{
            return false;
        }
    }
    ...
    /**
     * 由标识删除指定ID数据对象 :品牌商品分类关系
     * @param int $id 数据对象:品牌商品分类关系标识
     * @return boolen 是否删除成功；true为操作正常
     */
    public function deleteByID($id)
    {
        return Brandptype::deleteByID($id);
    }
    ...
    {
        return Brandptype::deleteByIds($ids);
    }

    /**
     * 对数据对象:品牌商品分类关系的属性进行递增
    ...
    {
        return Brandptype::increment($filter,$property_name,$incre_value);
    }

    /**
     * 对数据对象:品牌商品分类关系的属性进行递减
     * @param object|string|array $filter 查询条件，在where后的条件<br/>
     * 示例如下：<br/>
     * 0."id=1,name='sky'"<br/>
     * 1.array("id=1","name='sky'")<br/>
    ...
    {
        return Brandptype::decrement($filter,$property_name,$decre_value);
    }

    /**
     * 查询数据对象:品牌商品分类关系需显示属性的列表
    ...
    {
        return Brandptype::get($filter,$sort,$limit);
    }

    /**
     * 查询得到单个数据对象:品牌商品分类关系实体
     * @param object|string|array $filter 查询条件，在where后的条件<br/>
     * 示例如下：<br/>
     * 0."id=1,name='sky'"<br/>
     * 1.array("id=1","name='sky'")<br/>
    ...
    {
        return Brandptype::get_by_id($id);
    }
     ...
     * 3.允许对象如new User(id="1",name="green");<br/>
     * 默认:SQL Where条件子语句。如："(id=1 and name='sky') or (name like 'sky')"<br/>
     * @return 数据对象:品牌商品分类关系总计数
     */
    public function count($filter=null)
    {
        return Brandptype::count($filter);
    }
    ...
    {
        return Brandptype::queryPage($startPoint,$endPoint,$filter,$sort);
    }
    /**
     * 直接执行SQL语句
     * @return array
    ...
    {
        return self::dao()->sqlExecute("select * from ".Brandptype::tablename(),Brandptype::classname_static());
    }
}
?>