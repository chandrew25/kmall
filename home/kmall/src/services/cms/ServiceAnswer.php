 ...
 */
class ServiceAnswer extends Service implements IServiceBasic
{
    /**
     * 保存数据对象:客户商务定制
    ...
    /**
     * 由标识删除指定ID数据对象 :客户商务定制
     * @param int $id 数据对象:客户商务定制标识
     * @return boolen 是否删除成功；true为操作正常
     */
    public function deleteByID($id)
    {
        return Answer::deleteByID($id);
    }
    ...
    {
        return Answer::increment($filter,$property_name,$incre_value);
    }
    ...
    {
        return Answer::decrement($filter,$property_name,$decre_value);
    }

    /**
     * 查询数据对象:客户商务定制需显示属性的列表
    ...
    {
        return Answer::get($filter,$sort,$limit);
    }
     ...
     * 3.允许对象如new User(id="1",name="green");<br/>
     * 默认:SQL Where条件子语句。如："(id=1 and name='sky') or (name like 'sky')"<br/>
     * @return 单个数据对象:客户商务定制实体
     */
    public function get_one($filter=null)
    {
        return Answer::get_one($filter);
    }
     ...
     * 3.允许对象如new User(id="1",name="green");<br/>
     * 默认:SQL Where条件子语句。如："(id=1 and name='sky') or (name like 'sky')"<br/>
     * @return 数据对象:客户商务定制总计数
     */
    public function count($filter=null)
    {
        return Answer::count($filter);
    }
    ...
    {
        return Answer::queryPage($startPoint,$endPoint,$filter,$sort);
    }
    /**
     * 直接执行SQL语句
     * @return array
    ...
    {
        return self::dao()->sqlExecute("select * from ".Answer::tablename(),Answer::classname_static());
    }
}
?>