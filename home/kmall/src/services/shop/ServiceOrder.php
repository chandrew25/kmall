 ...
 */
class ServiceOrder extends Service implements IServiceBasic 
{
    /**
     * 保存数据对象:易乐会员的订单
    ...
    /**
     * 由标识删除指定ID数据对象 :易乐会员的订单
     * @param int $id 数据对象:易乐会员的订单标识
    ...
    {
        return Order::deleteByIds($ids);
    }

    /**
     * 对数据对象:易乐会员的订单的属性进行递增
    ...
    {
        return Order::increment($filter,$property_name,$incre_value);
    }

    /**
     * 对数据对象:易乐会员的订单的属性进行递减
     * @param string $filter 查询条件，在where后的条件<br/> 
     * 示例如下：<br/>
     * 0."id=1,name='sky'"<br/>
     * 1.array("id=1","name='sky'")<br/> 
    ...
    {
        return Order::decrement($filter,$property_name,$decre_value);
    }

    /**
     * 查询数据对象:易乐会员的订单需显示属性的列表
    ...
    {
        return Order::get($filter,$sort,$limit);
    }

    /**
     * 查询得到单个数据对象:易乐会员的订单实体
     * @param string $filter 查询条件，在where后的条件<br/> 
     * 示例如下：<br/>
     * 0."id=1,name='sky'"<br/>
     * 1.array("id=1","name='sky'")<br/> 
     ...
     * 3.允许对象如new User(id="1",name="green");<br/>
     * 默认:SQL Where条件子语句。如："(id=1 and name='sky') or (name like 'sky')"<br/> 
     * @return 单个数据对象:易乐会员的订单实体
     */
    public function get_one($filter=null)
    {
        return Order::get_one($filter);
    }
    ...
    {
        return self::dao()->sqlExecute("select * from ".Order::tablename(),Order::classname_static());
    }
    /**
     * 直接执行SQL语句
     * @return array
     ...
     *  2.执行更新和删除SQL语句返回执行成功与否的true|null
     */
    public static function sqlExecutes($filter,$object)
    {
        return self::dao()->sqlExecute($filter,$object);
    }
}
?>