 ...
 */
class ServiceOrderproducts extends Service implements IServiceBasic
{
    /**
     * 保存数据对象:订购商品
     * @param array|DataObject $orderproducts
     * @return int 保存对象记录的ID标识号
     */
    public function save($orderproducts)
    {
        if (is_array($orderproducts)){
            $orderproducts=new Orderproducts($orderproducts);
        }
        if ($orderproducts instanceof Orderproducts){
            return $orderproducts->save();
        }else{
            return false;
        }
    }
     ...
     */
    public function update($orderproducts)
    {
        if (is_array($orderproducts)){
            $orderproducts=new Orderproducts($orderproducts);
        }
    ...
    /**
     * 由标识删除指定ID数据对象 :订购商品
     * @param int $id 数据对象:订购商品标识
     * @return boolen 是否删除成功；true为操作正常
     */
    public function deleteByID($id)
    {
        return Orderproducts::deleteByID($id);
    }
    ...
    {
        return Orderproducts::deleteByIds($ids);
    }
    ...
    {
        return Orderproducts::increment($filter,$property_name,$incre_value);
    }
    ...
    {
        return Orderproducts::decrement($filter,$property_name,$decre_value);
    }
    ...
    {
        return Orderproducts::get($filter,$sort,$limit);
    }
     ...
     * 3.允许对象如new User(id="1",name="green");<br/>
     * 默认:SQL Where条件子语句。如："(id=1 and name='sky') or (name like 'sky')"<br/>
     * @return 单个数据对象:订购商品实体
     */
    public function get_one($filter=null)
    {
        return Orderproducts::get_one($filter);
    }
    ...
    {
        return Orderproducts::get_by_id($id);
    }
    ...
    {
        return Orderproducts::count($filter);
    }
    ...
    {
        return Orderproducts::queryPage($startPoint,$endPoint,$filter,$sort);
    }
    /**
     * 直接执行SQL语句
     * @return array
    ...
    {
        return self::dao()->sqlExecute("select * from ".Orderproducts::tablename(),Orderproducts::classname_static());
    }
}
?>