 ...
 */
class ServiceBtype extends Service implements IServiceBasic
{
    /**
     * 保存数据对象:业务类型
     * @param array|DataObject $btype
     * @return int 保存对象记录的ID标识号
     */
    public function save($btype)
    {
        if (is_array($btype)){
            $btype=new Btype($btype);
        }
        if ($btype instanceof Btype){
            return $btype->save();
        }else{
            return false;
        }
    ...
    /**
     * 由标识删除指定ID数据对象 :业务类型
     * @param int $id 数据对象:业务类型标识
     * @return boolen 是否删除成功；true为操作正常
     */
    public function deleteByID($id)
    {
        return Btype::deleteByID($id);
    }

    /**
     * 根据主键删除数据对象:业务类型的多条数据记录
    ...
    {
        return Btype::increment($filter,$property_name,$incre_value);
    }
    ...
    {
        return Btype::decrement($filter,$property_name,$decre_value);
    }
    ...
    {
        return Btype::get($filter,$sort,$limit);
    }
     ...
     * 3.允许对象如new User(id="1",name="green");<br/>
     * 默认:SQL Where条件子语句。如："(id=1 and name='sky') or (name like 'sky')"<br/>
     * @return 单个数据对象:业务类型实体
     */
    public function get_one($filter=null)
    {
        return Btype::get_one($filter);
    }
    ...
    {
        return self::dao()->sqlExecute("select * from ".Btype::tablename(),Btype::classname_static());
    }
}
?>