 ...
 */
class ServicePaymenttype extends Service implements IServiceBasic
{
    /**
     * 保存数据对象:支付方式
    ...
    /**
     * 由标识删除指定ID数据对象 :支付方式
     * @param int $id 数据对象:支付方式标识
     * @return boolen 是否删除成功；true为操作正常
     */
    public function deleteByID($id)
    {
        return Paymenttype::deleteByID($id);
    }

    /**
     * 根据主键删除数据对象:支付方式的多条数据记录
    ...
    {
        return Paymenttype::deleteByIds($ids);
    }
    ...
    {
        return Paymenttype::increment($filter,$property_name,$incre_value);
    }
    ...
    {
        return Paymenttype::decrement($filter,$property_name,$decre_value);
    }
    ...
    {
        return Paymenttype::get($filter,$sort,$limit);
    }
     ...
     * 3.允许对象如new User(id="1",name="green");<br/>
     * 默认:SQL Where条件子语句。如："(id=1 and name='sky') or (name like 'sky')"<br/>
     * @return 单个数据对象:支付方式实体
     */
    public function get_one($filter=null)
    {
        return Paymenttype::get_one($filter);
    }
    ...
    {
        return Paymenttype::get_by_id($id);
    }
    ...
    {
        return Paymenttype::count($filter);
    }
    ...
    {
        return Paymenttype::queryPage($startPoint,$endPoint,$filter,$sort);
    }
    /**
     * 直接执行SQL语句
     * @return array
    ...
    {
        return self::dao()->sqlExecute("select * from ".Paymenttype::tablename(),Paymenttype::classname_static());
    }
}
?>