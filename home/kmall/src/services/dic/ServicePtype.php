 ...
 */
class ServicePtype extends Service implements IServiceBasic
{
    /**
     * 保存数据对象:商品类型
    ...
    /**
     * 由标识删除指定ID数据对象 :商品类型
     * @param int $id 数据对象:商品类型标识
     * @return boolen 是否删除成功；true为操作正常
     */
    public function deleteByID($id)
    {
        return Ptype::deleteByID($id);
    }
    ...
    {
        return Ptype::increment($filter,$property_name,$incre_value);
    }
    ...
    {
        return Ptype::decrement($filter,$property_name,$decre_value);
    }
    ...
    {
        return Ptype::get($filter,$sort,$limit);
    }
     ...
     * 3.允许对象如new User(id="1",name="green");<br/>
     * 默认:SQL Where条件子语句。如："(id=1 and name='sky') or (name like 'sky')"<br/>
     * @return 单个数据对象:商品类型实体
     */
    public function get_one($filter=null)
    {
        return Ptype::get_one($filter);
    }
    ...
    {
        return Ptype::queryPage($startPoint,$endPoint,$filter,$sort);
    }
    /**
     * 直接执行SQL语句
     * @return array
    ...
    {
        return self::dao()->sqlExecute("select * from ".Ptype::tablename(),Ptype::classname_static());
    }
}
?>